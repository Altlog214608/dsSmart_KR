...
from PySide6.QtWidgets import QApplication, QWidget, QLineEdit, \
    QTableWidgetItem, QScroller, QAbstractItemView, QFileDialog
...
class UiDlg(QWidget):
    ...
    def uiDlgLogin(self, uiLoader):
        ...
        self.ui_dlg_login_resetpw.pb_cancel.clicked.connect(self.uiDlgLoginResetPWCancel)
        self.ui_dlg_login_resetpw.le_pw_old.installEventFilter(self.filter_ReturnTabSpace)
        self.ui_dlg_login_resetpw.le_pw_new.installEventFilter(self.filter_ReturnTabSpace)
        self.ui_dlg_login_resetpw.le_pw_new_check.installEventFilter(self.filter_ReturnTabSpace)
    ...
    def uiDlgSubject(self, uiLoader):
        self.ui_subject_dlg = uiLoader.load(
            "./ui/ui_subject.ui")
        self.ui_subject_dlg.pb_subject_search.clicked.connect(self.uiSubjectPbSubjectSearch)
        self.ui_subject_dlg.pb_subject_add.clicked.connect(self.uiSubjectPbSubjectAdd)
        self.ui_subject_dlg.pb_subject_delete.clicked.connect(self.uiSubjectPbSubjectDelete)
        ...
        self.ui_dlg_subject_add = uiLoader.load(
            "./ui/ui_dlg_subject_add.ui")
        ...
        self.ui_dlg_subject_delete = uiLoader.load(
            "./ui/ui_dlg_subject_delete.ui")
        self.ui_dlg_subject_delete.pb_delete.clicked.connect(self.uiDlgSubjectDeletePbDelete)
        self.ui_dlg_subject_delete.pb_close.clicked.connect(self.uiDlgSubjectDeletePbClose)
        self.setWindowBySetting(self.ui_dlg_subject_delete)

        # 피검자 정보 확인하고 다음으로 진행할지 확인하는 Dialog
        self.ui_dlg_subject_next = uiLoader.load(
            "./ui/ui_dlg_subject_next.ui")
    ...
    def setSubjectInfoToDlgs(self, name = "", birth_date = "", gender = ""):
        ...
        self.ui_test_identification_guide_picture.label_birth_date.setText(self.birth_date)
    ...
    def uiDlgTrainST(self, uiLoader):
        ...
        self.ui_train_st_response.label_selfcheck.installEventFilter(self.filter_ReturnTabSpace)
    ...
    def uiDlgMessages(self, uiLoader):
        # 알림 메시지 다이얼로그
        self.ui_msg_dlg = uiLoader.load(
            "./ui/ui_dlg_msg.ui")
        self.ui_msg_dlg.pb_close.clicked.connect(
            self.uiDlgMsgHide)
    ...
    def uiDlgSettings(self, uiLoader):
        ...
        self.ui_settings_dlg.pushButton_devices.clicked.connect(self.pushButton_devices_clicked_settings)
        self.ui_settings_dlg.pushButton_connect.clicked.connect(self.pushButton_connect_clicked_settings)
        self.ui_settings_dlg.comboBox_port.currentIndexChanged.connect(self.comboBox_port_changed_settings)
    ...
    def pushButton_devices_clicked_settings(self):
        ...
        self.ui_settings_dlg.label_connect.setText(
            {False: dsText.serialText['status_disconnect'], True: dsText.serialText['status_connect']}[dsSerial._is_open(self._serial)])
        self.ui_settings_dlg.pushButton_connect.setText(
            {False: dsText.serialText['try_connect'], True: dsText.serialText['try_disconnect']}[dsSerial._is_open(self._serial)])
    ...
    def parseReadData(self, data):
        ...
        try:
            self.rcv_id, self.rcv_func, self.rcv_address = struct.unpack_from('>BBH', data, offset=0)
            self.ui_data_protocol_dlg.textEdit_console.append("ID:%d, FUNC:%d, ADDR:%d" % self.rcv_id, self.rcv_func, self.rcv_address)
    ...
    def progressBarScentAndCleanForTrainST(self, scent_no, progress_bar, label_text):
        # 명령 전달
        self.requestScentNoAndTime(scent_no, command=dsComm.CMD_RUN_CLEAR, scent_run_time=15)
        scent_time = 15
        for i in range(1, 101):
            QtTest.QTest.qWait(scent_time * 9)
            progress_bar.setValue(i)
    ...
    def updateUiSujbect(self):
        ...
        self.ui_subject_dlg.table_subject.verticalHeader().setVisible(False) # 행 인덱스 안보이게
        ...
        self.ui_subject_dlg.table_subject.setEditTriggers(QAbstractItemView.NoEditTriggers) # 수정 안되게
        self.ui_subject_dlg.table_subject.setColumnCount(len(data_subjects[0])-1)  #  ID 제외하려면, (-1)
        ...
        for id, name, birth_date, gender in data_subjects:
            if subjectIndex > 0:
                ...
                self.ui_subject_dlg.table_subject.setItem(nRow, 0, QTableWidgetItem(name))
                self.ui_subject_dlg.table_subject.setItem(nRow, 1, QTableWidgetItem(birth_date))
                self.ui_subject_dlg.table_subject.setItem(nRow, 2, QTableWidgetItem(gender))
                self.ui_subject_dlg.table_subject.item(nRow, 0).setTextAlignment(Qt.AlignCenter | Qt.AlignVCenter)
                self.ui_subject_dlg.table_subject.item(nRow, 1).setTextAlignment(Qt.AlignCenter | Qt.AlignVCenter)
                self.ui_subject_dlg.table_subject.item(nRow, 2).setTextAlignment(Qt.AlignCenter | Qt.AlignVCenter)
            subjectIndex += 1

        self.ui_subject_dlg.table_subject.itemClicked.connect(self.cellSubjectTableCurrent)
        self.ui_subject_dlg.table_subject.setSelectionMode(QAbstractItemView.SelectionMode.SingleSelection)
        self.ui_subject_dlg.table_subject.setSelectionBehavior(QAbstractItemView.SelectionBehavior.SelectRows) # 선택: 행
    ...
    def updateUiSubjectDelete(self, name, birth_date, gender):
        self.ui_dlg_subject_delete.label_info.setText("이름: %s\n생년월일: %s" % (name, birth_date))
    ...
    def updateUiSubjectNext(self, name, birth_date, gender):
        self.ui_dlg_subject_next.label_info.setText("이름: %s\n생년월일: %s" % (name, birth_date))

    def uiDlgSubjectNextPbNext(self):
        print("uiDlgSubjectNextPbNext")
        self.uiDlgChangeWithDlg(self.ui_subject_dlg, self.ui_dlg_subject_next, self.ui_menu_dlg)
    ...
    def cellTestIDTableCurrent(self, item):
        if self.isTestIDChanging == False:
            ...
            test_date_time = self.ui_subject_dlg.table_test_id.item(item.row(), 3).text()
            self.setRecordSubjectInfo(name, birth_date, gender, test_date_time)
            self.makeSubjectTestRecordIdentification(name, birth_date, gender, test_date_time)
    ...
    def dbLoadTestId(self, db_results):
        ...
        score = 0
        for num in range(1, 1+len(dsTestID.id_test_data)): # 12문항이므로 1+12 까지
            if db_results[db_offset+((num*2)-1)] == db_results[db_offset+(num*2)]:
                is_choice_correct = 1
            else:
                is_choice_correct = 0
            id_results.append([num,
...
# [3, '녹차', '초콜릿', 0, '주관식X', 0]
    ...
    def makeSubjectTestRecordIdentification(self, name, birth_date, gender, test_date_time):
        print("makeSubjectTestRecordIdentification")
        # update table_suject
        try:
            db_id_results = dsTestDB.selectTableTestIDOne(name, birth_date, gender, test_date_time)[0] # DB에서 읽기
        except Exception as err:
        ...
        # 표 초기화
        self.ui_test_identification_record.resultTable.verticalHeader().setVisible(False) # 번호
        self.ui_test_identification_record.resultTable.clear()
        self.ui_test_identification_record.resultTable.setRowCount(0)
        self.ui_test_identification_record.resultTable.setEditTriggers(QAbstractItemView.NoEditTriggers)
        self.ui_test_identification_record.resultTable.setColumnCount(
            len(self.record_id_results[0]) - 3)  # 문항, 주관식 제외(-3)
        self.ui_test_identification_record.resultTable.setHorizontalHeaderLabels(
            [self.record_id_results[0][1], self.record_id_results[0][2],
             self.record_id_results[0][3]])
        self.ui_test_identification_record.resultTable.setColumnWidth(0, 300)  # 1번째 열 크기 늘임
        self.ui_test_identification_record.resultTable.setColumnWidth(1, 300)  # 2번째 열 크기 늘임
        self.ui_test_identification_record.resultTable.setColumnWidth(2, 210)  # 3번째 열 크기 늘임
        # Width 825 맞추면 됨
        count_correct = 0
        count_not_correct = 0
        tableIndex = 0
        for index, answer, choice_response, is_choice_correct, str_response, is_str_correct \
            ...
            if tableIndex > 0:
                ...
                self.ui_test_identification_record.resultTable.setItem(nRow, 0, QTableWidgetItem(answer))
                self.ui_test_identification_record.resultTable.setItem(nRow, 1, QTableWidgetItem(choice_response))
                self.ui_test_identification_record.resultTable.setItem(nRow, 2, QTableWidgetItem(
                    dsUtils.isCorrectToOX(is_choice_correct)))
                self.ui_test_identification_record.resultTable.item(nRow, 0).setTextAlignment(
                    Qt.AlignCenter | Qt.AlignVCenter)
                self.ui_test_identification_record.resultTable.item(nRow, 1).setTextAlignment(
                    Qt.AlignCenter | Qt.AlignVCenter)
                self.ui_test_identification_record.resultTable.item(nRow, 2).setTextAlignment(
                    Qt.AlignCenter | Qt.AlignVCenter)
                # 차트에 넣을 데이터 
        ...
        # 차트 (인지 검사는 파이 차트)
        if tableIndex > 1:
            self.ui_test_identification_record.widgetChart.apply_pie_chart(count_not_correct, count_correct)
        ...
        self.ui_test_identification_record.label_correctRate.setText(
            "%d" % correct_pct)
            # "%1.1f" % correct_pct)
    ...
    def updateUiSujbectTestIDAll(self):
        ...
        self.ui_subject_dlg.table_test_id.clear()
        self.ui_subject_dlg.table_test_id.verticalHeader().setVisible(False) # 행 인덱스 안보이게
        # self.ui_subject_dlg.table_test_id.horizontalHeader().setVisible(False) # 열 인덱스 안보이게
        self.ui_subject_dlg.table_test_id.setRowCount(0)
        self.ui_subject_dlg.table_test_id.setEditTriggers(QAbstractItemView.NoEditTriggers) # 수정 안되게
        self.ui_subject_dlg.table_test_id.setColumnCount(len(data_test_id[0])-25)  #  ID, 문제정보보 제외하려면, (-1)
        ...
        for id, name, birth_date, gender, test_date_time, test_score,\
            ...
            choice_07, choice_08, choice_09, choice_10, choice_11, choice_12 in data_test_id:
            if subjectIndex > 0:
                nRow = self.ui_subject_dlg.table_test_id.rowCount()
                self.ui_subject_dlg.table_test_id.setRowCount(nRow + 1)
                self.ui_subject_dlg.table_test_id.setItem(nRow, 0, QTableWidgetItem(name))
                self.ui_subject_dlg.table_test_id.setItem(nRow, 1, QTableWidgetItem(birth_date))
                self.ui_subject_dlg.table_test_id.setItem(nRow, 2, QTableWidgetItem(gender))
                self.ui_subject_dlg.table_test_id.setItem(nRow, 3, QTableWidgetItem(test_date_time))
                self.ui_subject_dlg.table_test_id.setItem(nRow, 4, QTableWidgetItem(str(test_score)))
                self.ui_subject_dlg.table_test_id.item(nRow, 0).setTextAlignment(Qt.AlignCenter | Qt.AlignVCenter)
                self.ui_subject_dlg.table_test_id.item(nRow, 1).setTextAlignment(Qt.AlignCenter | Qt.AlignVCenter)
                self.ui_subject_dlg.table_test_id.item(nRow, 2).setTextAlignment(Qt.AlignCenter | Qt.AlignVCenter)
                self.ui_subject_dlg.table_test_id.item(nRow, 3).setTextAlignment(Qt.AlignCenter | Qt.AlignVCenter)
                self.ui_subject_dlg.table_test_id.item(nRow, 4).setTextAlignment(Qt.AlignCenter | Qt.AlignVCenter)
            subjectIndex += 1

        # self.ui_subject_dlg.table_test_id.itemDoubleClicked.connect(self.cellTestIDTableCurrent)
        self.ui_subject_dlg.table_test_id.setSelectionMode(QAbstractItemView.SelectionMode.SingleSelection)
        self.ui_subject_dlg.table_test_id.setSelectionBehavior(QAbstractItemView.SelectionBehavior.SelectRows) # 선택: 행
    ...
    def uiMenuBtnTestResults(self):
        ...
        self.ui_test_results.resultTableT.setEditTriggers(QAbstractItemView.NoEditTriggers)
        ...
        self.ui_test_results.resultTableD.setEditTriggers(QAbstractItemView.NoEditTriggers)
        ...
        self.ui_test_results.resultTableI.setEditTriggers(QAbstractItemView.NoEditTriggers)
    ...
    def makeTestResultsThreshold(self):
        ...
        self.ui_test_threshold_results.resultTable.setEditTriggers(QAbstractItemView.NoEditTriggers)
    ...
    def makeTestResultsDiscrimination(self):
        ...
        self.ui_test_discrimination_results.resultTable.setEditTriggers(QAbstractItemView.NoEditTriggers)
    ...
    def makeTestResultsIdentification(self):
        ...
        self.ui_test_identification_results.resultTable.setEditTriggers(QAbstractItemView.NoEditTriggers)
    ...
    def uiTestIdentificationRecordSave(self):
        try:
            ...
            file_path, _ = QFileDialog.getSaveFileName(
                None, "파일 저장", file_name, "Excel 통합 문서 (*.xlsx);;모든 파일 (*.*)", options=options
            )
    ...
    def makeTrainSTResults(self):
        ...
        self.ui_train_st_results.resultTable.setEditTriggers(QAbstractItemView.NoEditTriggers)
    ...
    def uiSettingsScentPowerChanged(self):
        ...
        self.ui_settings_dlg.sb_scent_power.setValue(self.ui_settings_dlg.hs_scent_power.value())
    ...
    def uiSettingsScentRunTimeChanged(self):
        ...
        self.ui_settings_dlg.sb_scent_run_time.setValue(self.ui_settings_dlg.hs_scent_run_time.value())
    ...
    def uiSettingsCleaningPowerChanged(self):
        ...
        self.ui_settings_dlg.sb_cleaning_power.setValue(self.ui_settings_dlg.hs_cleaning_power.value())
    ...
    def uiSettingsCleaningRunTimeChanged(self):
        ...
        self.ui_settings_dlg.sb_cleaning_run_time.setValue(self.ui_settings_dlg.hs_cleaning_run_time.value())


    def uiSettingsSbScentPowerChanged(self):
        # self.ui_settings_dlg.te_scent_power.setPlainText(str(self.ui_settings_dlg.hs_scent_power.value()))
        self.ui_settings_dlg.hs_scent_power.setValue(self.ui_settings_dlg.sb_scent_power.value())

    def uiSettingsSbScentRunTimeChanged(self):
        # self.ui_settings_dlg.te_scent_run_time.setPlainText(str(self.ui_settings_dlg.hs_scent_run_time.value()))
        self.ui_settings_dlg.hs_scent_run_time.setValue(self.ui_settings_dlg.sb_scent_run_time.value())

    def uiSettingsSbCleaningPowerChanged(self):
        # self.ui_settings_dlg.te_cleaning_power.setPlainText(str(self.ui_settings_dlg.hs_cleaning_power.value()))
        self.ui_settings_dlg.hs_cleaning_power.setValue(self.ui_settings_dlg.sb_cleaning_power.value())

    def uiSettingsSbCleaningRunTimeChanged(self):
        # self.ui_settings_dlg.te_cleaning_run_time.setPlainText(str(self.ui_settings_dlg.hs_cleaning_run_time.value()))
        self.ui_settings_dlg.hs_cleaning_run_time.setValue(self.ui_settings_dlg.sb_cleaning_run_time.value())
    ...
    def updateSettingsUI(self):
        ...
        self.ui_settings_dlg.sb_scent_run_time.setValue(dsSetting.dsParam['scent_run_time'])
        ...
        self.ui_settings_dlg.sb_cleaning_power.setValue(dsSetting.dsParam['cleaning_power'])
        ...
        self.ui_settings_dlg.sb_cleaning_run_time.setValue(dsSetting.dsParam['cleaning_run_time'])
    ...
    def uiSettingUpdateSettings(self):
        ...
        dsSetting.dsParam['scent_run_time'] = self.ui_settings_dlg.sb_scent_run_time.value()
        dsSetting.dsParam['scent_post_delay'] = int(self.ui_settings_dlg.te_scent_post_delay.toPlainText())
        dsSetting.dsParam['cleaning_power'] = self.ui_settings_dlg.sb_cleaning_power.value()
        dsSetting.dsParam['cleaning_run_time'] = self.ui_settings_dlg.sb_cleaning_run_time.value()
    ...
    def saveDataThreshold(self):
        ...
        now = datetime.now()
        save_file = '%s\\' + dsText.resultText['results_data_raw_path'] + '\\%s_%s_%s_%04d년%02d월%02d일_%02d시%02d분_역치검사.xlsx' % (
            os.getcwd(), self.name, self.birth_date, self.gender, now.year, now.month, now.day, now.hour, now.minute)
    ...
    def saveDataDiscrimination(self):
        ...
        if not os.path.isdir(dsText.resultText['results_data_raw_path']):
            os.mkdir(dsText.resultText['results_data_raw_path'])
        now = datetime.now()
        save_file = '%s\\' + dsText.resultText['results_data_raw_path'] + '\\%s_%s_%s_%04d년%02d월%02d일_%02d시%02d분_식별검사.xlsx' % (
            os.getcwd(), self.name, self.birth_date, self.gender, now.year, now.month, now.day, now.hour, now.minute)
        workbook = xlsxwriter.Workbook(save_file)
        # 엑셀 채우기
        worksheet = workbook.add_worksheet()
    ...
    def saveDataIdentification(self):
        ...
        save_file = '%s\\' % os.getcwd() \
            ...
            self.name, self.birth_date, self.gender, now.year, now.month, now.day, now.hour, now.minute)
        workbook = xlsxwriter.Workbook(save_file)
        # 엑셀 채우기
        worksheet = workbook.add_worksheet()
        row = 0
    ...
    def saveReportIdentification(self, save_file):
        ...
        if len(self.record_id_results) > 1:
            ...
            for index, answer, choice_response, is_choice_correct, str_response, is_str_correct \
                ...
                m_col += 1
            # 점수 정보 넣기
            worksheet_main.write('M35', '=COUNTIF(B34:M34, "O")')
            worksheet_main.write('M36', '=COUNTIF(B34:M34, "X")')
    ...
    def saveDataResults(self):
        ...
        now = datetime.now()
        save_file = '%s\\'+ dsText.reportText['report_data_path'] + '\\%s_%s_%s_%04d년%02d월%02d일_%02d시%02d분_종합결과.xlsx' % (
            os.getcwd(), self.name, self.birth_date, self.gender, now.year, now.month, now.day, now.hour, now.minute)
        workbook = xlsxwriter.Workbook(save_file)
        ...
        worksheet_main.write('E5', dsText.reportText['report_birth_date'], format_small_title)
    ...
    def saveDataResultsTemp(self):
        ...
        now = datetime.now()
        save_file = '%s\\' + dsText.reportText['report_data_temp_path'] + '\\%s_%s_%s_%04d년%02d월%02d일_%02d시%02d분_종합결과.xlsx' % (
            os.getcwd(), self.name, self.birth_date, self.gender, now.year, now.month, now.day, now.hour, now.minute)
        workbook = xlsxwriter.Workbook(save_file)
